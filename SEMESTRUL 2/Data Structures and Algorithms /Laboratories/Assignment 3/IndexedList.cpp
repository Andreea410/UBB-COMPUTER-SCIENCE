...
/// WC: theta(n) (last element or nonexistent)
...
int IndexedList::valueBetweenLargestAndSmallest()
{
    int smallest_value = 1000000;
    int largest_value = -1;
    for (int i = 0; i < this->capacity; i++)
        if (this->array[i].info < smallest_value)
        {
            smallest_value = this->array[i].info;
        }
        else if (this->array[i].info > largest_value)
            largest_value = this->array[i].info;
