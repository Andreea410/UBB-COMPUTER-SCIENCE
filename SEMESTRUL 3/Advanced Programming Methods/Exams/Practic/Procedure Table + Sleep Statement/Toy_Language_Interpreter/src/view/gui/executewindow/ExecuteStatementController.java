...
public class ExecuteStatementController {
    ...
    @FXML
    private TableView<Map.Entry<String , MyPair<List<String> , IStmt>>> procedureTableView;
    @FXML
    private TableColumn<Map.Entry<String , MyPair<List<String> , IStmt>>,String> ProcedureName;
    @FXML
    private TableColumn<Map.Entry<String , MyPair<List<String> , IStmt>>,MyPair<List<String> , IStmt>> ProcedureBody;
    ...
    private void populatePrcedureTable() {
        PrgState currentProgramState = getCurrentProgramState();

        IMyProcedureTable procedureTable = Objects.requireNonNull(currentProgramState).getProcedureTable();

        ArrayList<Map.Entry<String, MyPair<List<String>, IStmt>>> procedureTableContent = new ArrayList<>();
        for (String key : procedureTable.getContent().keySet()) {
            MyPair<List<String>, IStmt> procedure = procedureTable.getContent().get(key);
            List<String> formalParams = procedure.getFirst();
            String procedureNameWithParams = key + "(" + String.join(", ", formalParams) + ")";
            procedureTableContent.add(Map.entry(procedureNameWithParams, procedure));
        }

        ObservableList<Map.Entry<String, MyPair<List<String>, IStmt>>> procedureTableData = FXCollections.observableArrayList(procedureTableContent);

        ProcedureName.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getKey()));

        ProcedureBody.setCellValueFactory(data -> new SimpleObjectProperty<>(data.getValue().getValue()));

        procedureTableView.setItems(procedureTableData);
    }
