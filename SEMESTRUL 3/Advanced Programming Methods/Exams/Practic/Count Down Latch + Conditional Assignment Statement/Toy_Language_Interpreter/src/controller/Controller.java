    ...
    public void allStep() throws InterruptedException{
        ...
        try {
            while (!programsList.isEmpty()) {
                ...
                repository.logPrgStateExec(programsList.get(0));
    ...
    public void OneStepForAllPrg(List<PrgState> prgStatess) throws ControllerException {
        ...
        try {
            newPrgList = executor.invokeAll(callableList).stream()
                    ...
                    .filter(Objects::nonNull).filter(pair -> pair.getKey() != null || pair.getValue() != null)
                    .toList();
        ...
        for (Pair error : newPrgList) {
            if (error.getValue() != null) {
                System.out.println("Error: " + error.getValue());
            }
        }

        prgStatess.addAll(newPrgList.stream().map(pair -> (PrgState) pair.getKey()).toList());
