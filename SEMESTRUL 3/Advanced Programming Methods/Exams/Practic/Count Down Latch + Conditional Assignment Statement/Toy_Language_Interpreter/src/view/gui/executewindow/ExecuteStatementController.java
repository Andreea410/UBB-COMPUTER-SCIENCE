...
public class ExecuteStatementController {
    ...
    @FXML
    private TableColumn<MyPair<Integer, Integer>, Integer> locationColumnLatchTable;
    @FXML
    private TableColumn<MyPair<Integer, Integer>, Integer> valueColumnLatchTable;
    @FXML
    private Label latchTableLabel;
    ...
    private void populateIdentifiers() {
        ...
        if (programStates != null) {
            for (PrgState prgState : programStates) {

                if (!prgState.getExeStack().isEmpty() && !identifiers.contains(prgState.getId())) {
    ...
    private void populateLatchTable() {
        ...
        if (currentProgramState == null) {
            showErrorMessage("No current program state available.");
            return;
        }

        var latchTable = currentProgramState.getLatchTable().getLatchtable();
        ObservableList<MyPair<Integer, Integer>> latchTableData = FXCollections.observableArrayList(
                latchTable.entrySet().stream()
                        .map(entry -> new MyPair<>(entry.getKey(), entry.getValue()))
                        .toList()
        );

        locationColumnLatchTable.setCellValueFactory(data -> new SimpleObjectProperty<>(data.getValue().getFirst()));
        valueColumnLatchTable.setCellValueFactory(data -> new SimpleObjectProperty<>(data.getValue().getSecond()));
