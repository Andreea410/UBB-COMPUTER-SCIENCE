...
public class ForkStatement implements IStmt
{
    ...
    public PrgState execute(PrgState prgState) throws StatementException, ADTException, IOException {
        ...
        MyDictionary<String , IValue> newSymTable = new MyDictionary<>();
        for(Map.Entry<String, IValue> entry : prgState.getSymTable().getContent().entrySet())
        {
            newSymTable.insert(entry.getKey(), entry.getValue());
        }
        return new PrgState(newExecutionStack, newSymTable, prgState.getOutput(), this.statement, prgState.getFileTable(), prgState.getHeap(), prgState.getSemaphoreTable());
    }
