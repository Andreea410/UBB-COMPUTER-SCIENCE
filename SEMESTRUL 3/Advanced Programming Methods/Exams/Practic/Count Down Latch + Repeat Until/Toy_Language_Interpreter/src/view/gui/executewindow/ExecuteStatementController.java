...
public class ExecuteStatementController {
    ...
    @FXML
    private TableColumn<MyPair<Integer , Integer>,Integer> latchIndexColumn;
    @FXML
    private TableColumn<MyPair<Integer , Integer>,Integer> latchValueColumn;
    ...
    private void populateLatchTable()
    {
        ...
        IMyLatchTable countdownLatchTable = Objects.requireNonNull(programState).getLatchTable();

        ArrayList<MyPair<Integer, Integer>> latchEntries = new ArrayList<>();

        for(Map.Entry<Integer, Integer> entry : countdownLatchTable.getLatchtable().entrySet())
            latchEntries.add(new MyPair<>(entry.getKey(), entry.getValue()));
